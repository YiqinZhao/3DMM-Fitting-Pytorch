...
def fit_coeffs(args, device, worker_ind):
    ...
    recon_model = get_recon_model(
        model=args.recon_model, device=device, batch_size=1, img_size=args.tar_size
    )
    recon_model.init_coeff_tensors(id_coeff=id_coeff, tex_coeff=tex_coeff)
    fitting_dataset = FittingDataset(
        args.tmp_face_folder,
        args.lm_pkl_path,
        worker_num=args.nworkers,
        worker_ind=worker_ind,
    )
    fitting_data_loader = torch.utils.data.DataLoader(
    ...
    for batch_ind, cur_batch in tqdm(enumerate(fitting_data_loader)):
        ...
        for i in range(num_iters):
            ...
            lm_loss_val = losses.lm_loss(
                lms_proj, lms, lm_weights, img_size=args.tar_size
            )
    ...
    with open(
        os.path.join(args.cache_folder, "%d_fitting.pkl" % worker_ind), "wb"
    ) as f:
...
def gen_composed_video(args, device):
    ...
    video = cv2.VideoWriter(
        ...
        video_info["fps"],
        (video_info["frame_w"], video_info["frame_h"]),
    )
    for k in tqdm(keys):
        orig_frame = cv2.imread(os.path.join(args.tmp_frame_folder, str(k) + ".png"))
        ...
        resized_mask = cv2.resize(out_mask, (face_w, face_h), cv2.INTER_NEAREST)[
            ..., None
        ]

        composed_face = (
            orig_frame[bbox[1] : bbox[3], bbox[0] : bbox[2], :] * (1 - resized_mask)
            + resized_out_img * resized_mask
        )
...
def fit_shape(args, device):
    ...
    fitting_dataset = FittingDataset(args.tmp_face_folder, args.lm_pkl_path)
    fitting_data_loader = torch.utils.data.DataLoader(
        dataset=fitting_dataset,
        batch_size=args.nframes_shape,
        shuffle=True,
    ...
    rigid_optimizer = torch.optim.Adam(
        [recon_model.get_rot_tensor(), recon_model.get_trans_tensor()], lr=args.rf_lr
    )
    ...
    for i in tqdm(range(args.first_nrf_iters)):
        ...
        photo_loss_val = losses.photo_loss(rendered_img[:, :, :, :3], imgs, mask > 0)
    ...
    np.save(
        args.id_npy_path,
        recon_model.get_id_tensor().detach().cpu().numpy().reshape(1, -1),
    )
    np.save(
        args.tex_npy_path,
        recon_model.get_tex_tensor().detach().cpu().numpy().reshape(1, -1),
    )
...
def process_video(args, device):
    ...
    print(
        "A face is detected. l: %d, t: %d, r: %d, b: %d"
    ...
    while cap.isOpened():
        ...
        face_out_path = os.path.join(args.tmp_face_folder, "%d.png" % frame_ind)
        frame_out_path = os.path.join(args.tmp_frame_folder, "%d.png" % frame_ind)
    ...
    cap.release()
    v_inf_dict = {"fps": fps, "frame_w": orig_w, "frame_h": orig_h, "bbox": bbox}
...
if __name__ == "__main__":
    ...
    processes = []
    for i in range(args.nworkers):
        p = Process(target=fit_coeffs, args=(args, args.devices[i % args.ngpus], i))
        p.start()
        processes.append(p)
