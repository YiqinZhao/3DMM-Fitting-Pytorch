...
import third_party.face_fitting_pytorch.core.losses as losses
import third_party.face_fitting_pytorch.core.utils as utils
from third_party.face_fitting_pytorch.core import get_recon_model
from third_party.face_fitting_pytorch.core.options import ImageFittingOptions
...
def fit(args):
    ...
    recon_model = get_recon_model(
        model=args.recon_model, device=args.device, batch_size=1, img_size=args.tar_size
    )
    ...
    with torch.no_grad():
        ...
        composed_face = (
            composed_img[bbox[1] : bbox[3], bbox[0] : bbox[2], :] * (1 - resized_mask)
        ...
        out_coeff_path = os.path.join(args.res_folder, basename + "_coeffs.npy")
        np.save(out_coeff_path, coeffs.detach().cpu().numpy().squeeze())

        # save the mesh into obj format
        out_obj_path = os.path.join(args.res_folder, basename + "_mesh.obj")
