...
class FittingDataset(data.Dataset):
    def __init__(self, img_folder, pkl_path, worker_num=1, worker_ind=0):
        ...
        for k in keys:
            ...
            assert os.path.exists(img_full_path), (
                "file %s does not exist" % img_full_path
            )
    ...
    def __getitem__(self, index):
        k, img_full_path = self.item_list[
            self.start_ind : self.start_ind + self.group_len
        ][index]
        lms = self.lm_dict[k]
