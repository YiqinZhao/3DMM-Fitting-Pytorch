...
class BaseReconModel(nn.Module):
    ...
    def project_vs(self, vs):
        ...
        vs = (
            torch.matmul(vs, self.reverse_z.repeat((batchsize, 1, 1))) + self.camera_pos
        )
    ...
    def get_packed_tensors(self):
        return self.merge_coeffs(
            self.id_tensor.repeat(self.batch_size, 1),
            self.exp_tensor,
    ...
    def init_coeff_tensors(self, id_coeff=None, tex_coeff=None):
        if id_coeff is None:
            ...
            self.id_tensor = torch.tensor(
                id_coeff, dtype=torch.float32, requires_grad=True, device=self.device
            )
        ...
        if tex_coeff is None:
            ...
            self.tex_tensor = torch.tensor(
                tex_coeff, dtype=torch.float32, requires_grad=True, device=self.device
            )
